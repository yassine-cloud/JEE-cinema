...
public class DaoProjection {
	...
	public ArrayList<Projection> getProjections() {
		...
		try {
			...
			while (rs.next()) {
				l.add(new Projection(rs.getInt(1), rs.getTimestamp(2).toLocalDateTime(), rs.getInt(3), rs.getInt(4),
						rs.getFloat(5)));
			}
			rs.close();
			ps.close();
	...
	public Projection getProjection(int id) {
		...
		try {
			...
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				p = new Projection(rs.getInt(1), rs.getTimestamp(2).toLocalDateTime(), rs.getInt(3), rs.getInt(4),
						rs.getFloat(5));
			}
			rs.close();
			ps.close();
